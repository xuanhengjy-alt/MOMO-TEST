console.log('🔍 Checking Phil personality test data...');
...
async function main() {
  ...
  try {
    ...
    if (projectResult.rows.length === 0) {
      console.log('❌ 菲尔人格测试项目不存在');
      return;
    }
    
    const project = projectResult.rows[0];
    console.log('✅ 菲尔人格测试项目存在:');
    ...
    if (firstQuestion.rows.length > 0) {
      ...
      firstQuestion.rows.forEach(row => {
        console.log(`   选项${row.option_number}: ${row.option_text_en} (得分: ${row.score_value})`);
      });
    }
