console.log('🔍 Checking Holland test data...');
...
async function main() {
  ...
  try {
    ...
    if (projectResult.rows.length === 0) {
      console.log('❌ 霍兰德职业兴趣测试项目不存在');
      return;
    }
    
    const project = projectResult.rows[0];
    console.log('✅ 霍兰德职业兴趣测试项目存在:');
    ...
    console.log(`   期望的选项数量: 180 (90题 × 2选项)`);
    ...
    console.log(`   期望的结果类型数量: 6`);
    
    if (resultTypeCount === 6) {
    ...
    // 检查第7题的详细信息（现实型题目）
    console.log('\n🔍 第7题详细信息（现实型题目）:');
    const question7 = await client.query(`
      SELECT q.question_number, q.question_text_en, 
             qo.option_number, qo.option_text_en, qo.score_value
      FROM questions q
      JOIN question_options qo ON q.id = qo.question_id
      WHERE q.project_id = $1 AND q.question_number = 7
      ORDER BY qo.option_number
    `, [project.id]);
    
    if (question7.rows.length > 0) {
      console.log(`   题目: ${question7.rows[0].question_text_en}`);
      question7.rows.forEach(row => {
        console.log(`   选项${row.option_number}: ${row.option_text_en} (分数: ${row.score_value})`);
      });
    }
    ...
    // 测试1: 全部选择Yes (1分)
    console.log('Test 1 - 全部选择Yes: 期望每种类型15分');
    
    // 测试2: 全部选择No (0分)
    console.log('Test 2 - 全部选择No: 期望每种类型0分');
